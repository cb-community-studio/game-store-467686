...
const GameCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await client.service("game").create(_data);
            props.onHide();
    ...
    return (
        <Dialog header="Create" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="game-create-dialog-component">
                <div>
                ...
                <div>
                    ...
                    <Calendar dateFormat="dd/mm/yy hh:mm" placeholder={"dd/mm/yy hh:mm"} value={_entity?.releaseDate} onChange={ (e) => setValByKey("releaseDate", e.target.value)} showTime ></Calendar>
                </div>
                <div>
                    <p className="m-0" >Price (RM):</p>
