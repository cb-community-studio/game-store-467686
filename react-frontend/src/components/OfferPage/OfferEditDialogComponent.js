...
const OfferCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        setLoading(true);
        try {
            const result = await client.service("offer").patch(_entity._id, _data);
            props.onHide();
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="offer-edit-dialog-component">
                <div>
                    <p className="m-0" >Promotion:</p>
                    <InputText className="w-full mb-3" value={_entity?.offerType} onChange={(e) => setValByKey("offerType", e.target.value)}  />
                </div>
                <div>
